...
#OCTOPUS Get files
```{r}
oc_get_file_DLC = function(WD,folder_date){ 
  wd = c(WD,"Raw Data\\Raw_Octopus_DLC_Label_Coordinates",folder_date)
...
#Read files
```{r}
oc_read_file_DLC = function(WD,folder_date,f){
  wd = c(WD,"Raw Data\\Raw_Octopus_DLC_Label_Coordinates",folder_date,f)
...
```{r}
oc_organize_coordinates_DLC = function(dat_tab){
  len = nrow(dat_tab)
  
  frame = c(dat_tab[3:len,1])

  left_eye_x = dat_tab[3:len,2]
  left_eye_y = dat_tab[3:len,3]
  left_eye_likelihood = dat_tab[3:len,4]
  left_eye = data.frame(left_eye_x,left_eye_y,left_eye_likelihood)
  
  right_eye_x = dat_tab[3:len,5]
  right_eye_y = dat_tab[3:len,6]
  right_eye_likelihood = dat_tab[3:len,7]
  right_eye = data.frame(right_eye_x,right_eye_y,right_eye_likelihood)

  coors = data.frame(frame,left_eye,right_eye)
...
main_loop = function(DATE,WORKD){
  #OCTOPUS
    #FIND FILES
  in_files_list = oc_get_file_DLC(WORKD,DATE)
  #print(in_files_list)
  
    #READ FILES
  for (i in in_files_list) {
    raw_csv = oc_read_file_DLC(WORKD,DATE,i)
  
    coor_temp = oc_organize_coordinates_DLC(raw_csv)
  ...
  #Rename variables
    ...
    write.csv(b_change_plu_s_peed,paste0(WORKD,"\\Processed Data\\Full Video Results DLC\\",DATE,"\\Processed_",i))
  }
}
```
...
for (i in 1:length(datelist_u)){
  ...
  path = paste0(WORKD,"\\Processed Data\\Full Video Results DLC")
  newfolder = DATE
