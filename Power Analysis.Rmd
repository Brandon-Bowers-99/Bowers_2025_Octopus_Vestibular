...
This first section is being used to determine what mean difference between treatment and control would be estimated in correlation with a power level of ~0.8.
...
temp.lme=lme(
             Avg_Speed_Per_Bodylength~Trial_Type*NewDates,
             random=~1|OC_ID,
             data=temporary
             )
...
## Average Change in B-H Diff
```{r}
FV_master_set$Trial_Type=as.factor(FV_master_set$Trial_Type)

FV_AVG_CBvH.lme=lme((Average_Change_Bearing_vs_Heading_FV)~Trial_Type*NewDates,random=~1|OC_ID,
                       data=FV_master_set)
...
obs_diff =  mean(FV_master_set$Average_Change_Bearing_vs_Heading_FV[FV_master_set$Trial_Type == "4"]) -
...
temporary$Average_Change_Bearing_vs_Heading_FV[FV_master_set$Trial_Type == "4"]=
  temporary$Average_Change_Bearing_vs_Heading_FV[FV_master_set$Trial_Type == "4"]+step

mean_diff=mean(temporary$Average_Change_Bearing_vs_Heading_FV[temporary$Trial_Type == "4"]) -
            mean(temporary$Average_Change_Bearing_vs_Heading_FV[temporary$Trial_Type == "1"])

temp.lme=lme(
             Average_Change_Bearing_vs_Heading_FV~Trial_Type*NewDates,
             random=~1|OC_ID,
             data=temporary
             )
...
obs_diff =  mean(PA_master_set$Bearing_Change[PA_master_set$Movement_Type == "1"&PA_master_set$Trial_Type == "4"]) -
            mean(PA_master_set$Bearing_Change[PA_master_set$Movement_Type == "1"&PA_master_set$Trial_Type == "1"])
...
temporary
temp.lme=lme((Bearing_Change)~Trial_Type*NewDates,
             random=~1|OC_ID,
             data=temporary)
