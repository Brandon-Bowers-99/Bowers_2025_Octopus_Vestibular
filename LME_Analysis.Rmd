...
predict(bearing_change.lme, PA_master_set)

p <- ggplot(PA_master_set, aes(x = NewDates, y = Movement_Type, colour = Trial_Type)) +
  geom_point(size=3) +
  geom_line(aes(y = predict(bearing_change.lme)),size=1) 
print(p)
...
dates.ex=strptime(PA_master_set$Date[Movement_Type == 2],"%m_%d")
NewDates = difftime(dates.ex,min(dates.ex),units="day")
...
p <- ggplot(PA_master_set[PA_master_set$Movement_Type==2,], aes(x = NewDates, y = Movement_Type, color = Trial_Type)) +
...
for (i in 1:length(NewDates)){
  Pre_TT_1 = c(Pre_TT_1, 1)
  Pre_TT_2 = c(Pre_TT_2, 2)
}
...
bearing_change.ANOVA=Anova(bearing_change.lme,type="III")
bearing_change.ANOVA
predict(bearing_change.lme, newdata = data.frame(NewDates = c(NewDates),Trial_Type = c(Pre_TT_1)))
```
...
NewDates = difftime(dates.ex,min(dates.ex),units="day")
FV_master_set$NewDates=as.numeric(NewDates)

#FV_master_set$Duration = as.numeric(FV_master_set$Duration)
