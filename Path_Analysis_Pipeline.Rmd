...
```{r}
DATE = "7_24"
datelist = c("7/24","7/28","7/30","8/1","8/3","8/5","8/7","8/9","8/11","8/13","8/15","8/17")
DATE2 = "7/24"
...
```{r}
path_deviance = function(DATE,date_batch,index){
  frames = c()
  total_movement_dis = c()
  angle_dev = c()
  bearing_movedir_diff = c()
  
  ocnum = start_end_frames$OCNUM[date_batch[index]]
  ...
  for (k in 1:length(ordered_curr_oc$X.1)){
    ...
    if (as.character(start_end_frames$START[i]) == as.character(ordered_curr_oc$X.1[k])){
      start_index = index_count
      index_count = index_count + 1
    }
    else if (as.character(start_end_frames$END[i]) == as.character(ordered_curr_oc$X.1[k])){
  ...
  angle_dev = c(angle_dev,real_vs_ideal)
  total_movement_dis = c(total_movement_dis,total_dis)
  duration = (start_end_frames$DURATION[date_batch[index]] / 60)
  ...
  ############
  return(c(total_movement_dis,angle_dev,duration,avg_bear_movedir_dif,avg_bear_movedir_dif_dif))
}
```
...
for (i in 1:length(start_end_frames$DATE)){
  if (start_end_frames$DATE[i] == DATE2){
...
for (j in 1:length(date_batch)){
  if (start_end_frames$MOVEMENT.TYPE[date_batch[j]] == "1"|start_end_frames$MOVEMENT.TYPE[date_batch[j]] == "2"){
    #returns (tot_move_dis,angle_dev) 
    path_dev = path_deviance(DATE,date_batch,j)
    ...
    path_analysis_results = data.frame(date,oc_ID,movement_type,total_movement_dis,duration,angle_dev,avg_bear_head_dif,avg_bear_head_dif_dif)  
   }

}

write.csv(path_analysis_results,paste0(WORKD,"\\Processed Data\\Path Analysis Results\\PA_Results_",DATE,".csv"))
```
